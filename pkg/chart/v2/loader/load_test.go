...
func TestLoadFiles_BadCases(t *testing.T) {
	for _, tt := range []struct {
		...
		require.Errorf(t, err, "expected error when load illegal files")
		assert.ErrorContainsf(t, err, tt.expectError, "Expected error to contain %q, got %q for %s", tt.expectError, err.Error(), tt.name)
	}
}
...
func verifyChart(t *testing.T, c *chart.Chart) {
	...
	for _, dep := range c.Dependencies() {
		...
		assert.Equalf(t, dep.Metadata.Version, exp["version"], "Expected %s version %s, got %s", dep.Name(), exp["version"], dep.Metadata.Version)
	}
}
