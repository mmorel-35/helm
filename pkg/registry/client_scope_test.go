...
func (suite *RegistryScopeTestSuite) Test_1_Check_Push_Request_Scope() {
	...
	suite.Require().NoErrorf(err, "no error creating server listener")
	...
	suite.Require().Errorf(err, "error pushing good ref because auth server doesn't give proper token")

	// check the url that authentication server received
	select {
	case urlStr := <-requestURL:
		u, err := url.Parse(urlStr)
		suite.Require().NoErrorf(err, "no error parsing requested URL")
...
func (suite *RegistryScopeTestSuite) Test_2_Check_Pull_Request_Scope() {
	...
	suite.Require().Errorf(err, "error pulling a simple chart because auth server doesn't give proper token")
